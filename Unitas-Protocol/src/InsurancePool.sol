...
contract InsurancePool is AccessControl, ReentrancyGuard, IInsurancePool, PoolBalances {
    ...
    function withdrawCollateral(address token, uint256 amount) external onlyGuardianOrWithdrawer nonReentrant {
        ...
        console.log('\n collateral ', collateral); // @audit _balance storage variable

        console.log('_getPortfolio(token) ',  _getPortfolio(token));
        console.log('collateral - _getPortfolio(token) ',  collateral - _getPortfolio(token));
        console.log('collateral - _getPortfolio(token) >= amount ',  collateral - _getPortfolio(token) >= amount);
        _require(collateral - _getPortfolio(token) >= amount, Errors.POOL_BALANCE_INSUFFICIENT);

        console.log('_setBalance ', collateral - amount);
     ...
     */
    function receivePortfolio(address token, uint256 amount) external onlyPortfolio(msg.sender) nonReentrant {
