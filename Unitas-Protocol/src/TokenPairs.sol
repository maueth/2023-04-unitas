...
abstract contract TokenPairs is ITokenPairs {
     ...
     */
    function listPairTokensByIndexAndCount(address token, uint256 index, uint256 count)
        external
        view
        virtual
        returns (address[] memory)
    {
        ...
        _require((index == 0 || index < tokenCount) && index + count <= tokenCount, Errors.INPUT_OUT_OF_BOUNDS);
    ...
    function _addPairByTokens(address tokenX, address tokenY) internal virtual returns (bytes32) {
        ...
        _require(_pairTokens[tokenX].add(tokenY) && _pairTokens[tokenY].add(tokenX), Errors.PAIR_ALREADY_EXISTS);
    ...
    function _removePairByTokens(address tokenX, address tokenY) internal virtual returns (bytes32) {
        ...
        _require(_pairTokens[tokenX].remove(tokenY) && _pairTokens[tokenY].remove(tokenX), Errors.PAIR_NOT_EXISTS);
