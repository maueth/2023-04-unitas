    ...
    Test
{
        ...
        returns (uint256 amountIn, uint256 amountOut)
    {
        ...
        console.log(IERC20(tokenIn).balanceOf(msg.sender), amountIn);
    ...
    function _swapOut(address token, address receiver, uint256 amount) internal {
        ...
        */
        ...
        if (tokenType == ITokenManager.TokenType.Asset) {
            console.log('tokenType Asset');

            uint256 tokenReserve = _getBalance(token); // @audit IP Pool _balance storage variable
            uint256 reserveAmount = amount.min(tokenReserve - _getPortfolio(token)); // @audit Insurance Pool _portfolio sotage variable

            console.log('amount ', amount);
            console.log('tokenReserve (IP Pool) ', tokenReserve);
            console.log('_getPortfolio(token) (Insurance Pool)', _getPortfolio(token));
            console.log('tokenReserve - Insurance Pool ', tokenReserve - _getPortfolio(token));
            console.log('min ', reserveAmount);
            console.log('amount > reserveAmount ', amount > reserveAmount);

            if (amount > reserveAmount) {
                uint256 collateralAmount = amount - reserveAmount;
                console.log('collateralAmount ', collateralAmount); // @audit get difference between IP and Insurance Pool

                // Pull the collateral from insurance pool
                IInsurancePool(insurancePool).withdrawCollateral(token, collateralAmount);
            }

            console.log('\n _setBalance ', tokenReserve - reserveAmount);
            
            _setBalance(token, tokenReserve - reserveAmount);
            IERC20(token).safeTransfer(receiver, amount);
        } else {
            console.log('tokenType Stable');
