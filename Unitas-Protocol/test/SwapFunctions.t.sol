...
contract SwapFunctionsTest is Test {
    ...
    function test_getFeeByAmountWithFee_Correct() public {
        assertEq(_swapFunctions.exposed_getFeeByAmountWithFee(1e18, 0, 0), 0, "0");

        assertEq(_swapFunctions.exposed_getFeeByAmountWithFee(1e18, 0.01e6, 1e6), 0.01e18, "1%");

        // 100000 * 0.9999 = 99990
        assertEq(_swapFunctions.exposed_getFeeByAmountWithFee(100000e18, 0.9999e6, 1e6), 99990e18, "99.99%");
    ...
    function test_getFeeByAmountWithoutFee_Correct() public {
        assertEq(_swapFunctions.exposed_getFeeByAmountWithoutFee(1e18, 0, 0), 0, "0");

        assertEq(_swapFunctions.exposed_getFeeByAmountWithoutFee(1e18, 0.01e6, 1e6), 0.010101010101010102e18, "1% ceil");

        // 10 / (1 - 0.9999) = 100000
        // 100000 - 10 = 99990
        assertEq(_swapFunctions.exposed_getFeeByAmountWithoutFee(10e18, 0.9999e6, 1e6), 99990e18, "99.99%");
    ...
    function test_convert_FailWhenQuoteTokenInvalid() public {
        ...
        vm.expectRevert(message);
        _swapFunctions.exposed_convert(
            address(_usdt), address(_usd1), 1e18, MathUpgradeable.Rounding.Down, 1e18, 1e18, address(0x0)
        );

        vm.expectRevert(message);
        _swapFunctions.exposed_convert(
            address(_usdt), address(_usd1), 1e18, MathUpgradeable.Rounding.Down, 1e18, 1e18, address(_usd91)
        );
    }
    ...
    function test_convert_WhenTokensSame() public {
        ...
        uint256 toAmount = _swapFunctions.exposed_convert(
            address(_usd1), address(_usd1), fromAmount, MathUpgradeable.Rounding.Up, 1e18, 1e18, address(0x0)
        );
        assertEq(toAmount, fromAmount, "to amount when round up");

        toAmount = _swapFunctions.exposed_convert(
            address(_usd1), address(_usd1), fromAmount, MathUpgradeable.Rounding.Down, 1e18, 1e18, address(_usd91)
        );
    ...
    function test_convert_WhenFromAmountZero() public {
        ...
        uint256 toAmount = _swapFunctions.exposed_convert(
            address(_usd1), address(_usd91), fromAmount, MathUpgradeable.Rounding.Up, price, 1e18, address(_usd91)
        );
        assertEq(toAmount, fromAmount, "to amount when usd1 to usd91 with round up");

        toAmount = _swapFunctions.exposed_convert(
            address(_usd1), address(_usd91), fromAmount, MathUpgradeable.Rounding.Down, price, 1e18, address(_usd91)
        );
        assertEq(toAmount, fromAmount, "to amount when usd1 to usd91 with round down");

        toAmount = _swapFunctions.exposed_convert(
            address(_usd91), address(_usd1), fromAmount, MathUpgradeable.Rounding.Up, price, 1e18, address(_usd91)
        );
        assertEq(toAmount, fromAmount, "to amount when usd91 to usd1 with round up");

        toAmount = _swapFunctions.exposed_convert(
            address(_usd91), address(_usd1), fromAmount, MathUpgradeable.Rounding.Down, price, 1e18, address(_usd91)
        );
    ...
    function test_convertByToPrice_FailWhenPriceZero() public {
        vm.expectRevert();
        _swapFunctions.exposed_convertByToPrice(
            address(_usdt), address(_usd1), 1e18, MathUpgradeable.Rounding.Down, 0, 1e18
        );
    }
    ...
    function testFuzz_convertByFromPrice_CorrectWhenScaledPriceDecimals(
        uint128 fromAmount,
        uint96 fromPrice,
        uint8 priceDecimals
    ) public {
    ...
    function testFuzz_convertByToPrice_CorrectWhenScaledPriceDecimals(
        uint128 fromAmount,
        uint96 toPrice,
        uint8 priceDecimals
    ) public {
    ...
    function _assertConvertCorrect(
        ...
        uint256 toAmount = _swapFunctions.exposed_convert(
            fromToken, toToken, fromAmount, rounding, price, 10 ** priceDecimals, quoteToken
        );
    ...
    function _assertConvertByFromPriceCorrect(
        ...
        _assertConvertByFromPriceCorrect(
            address(fromToken), address(toToken), fromAmount, rounding, fromPrice, priceDecimals
        );
    }
    ...
    function _assertConvertByFromPriceCorrect(
        ...
        uint256 toAmount = _swapFunctions.exposed_convertByFromPrice(
            fromToken, toToken, fromAmount, rounding, fromPrice, 10 ** priceDecimals
        );
    ...
    function _assertConvertByToPriceCorrect(
        ...
        _assertConvertByToPriceCorrect(
            address(fromToken), address(toToken), fromAmount, rounding, toPrice, priceDecimals
        );
    }
    ...
    function _assertConvertByToPriceCorrect(
        ...
        uint256 toAmount = _swapFunctions.exposed_convertByToPrice(
            fromToken, toToken, fromAmount, rounding, toPrice, 10 ** priceDecimals
        );
...
contract SwapFunctionsHarness is SwapFunctions {
    ...
    function exposed_validateFeeFraction(uint256 numerator, uint256 denominator) external view {
