...
contract UnitasTest is Test {
    ...
    function test_pause_FailWhenNotGuardian() public {
        vm.expectRevert(abi.encodeWithSignature("NotGuardian(address)", address(this)));
        _unitas.pause();
    }
    ...
    function test_swap_FailWhenCollateralInsufficientToRedeemAssetTokenByReserveNotEnough() public {
        ...
        _unitas.swap(address(_usd1), address(_usdt), ISwapFunctions.AmountType.In, usd1Amount);
    }
    ...
    function test_swap_FailWhenCollateralInsufficientToRedeemAssetTokenByStableTokenAppreciation() public {
        ...
        _unitas.swap(
            ...
            ScalingUtils.scaleByDecimals(100e18, 18, _usdt.decimals())
        );
    ...
    function test_swap_WhenMintUSD1WithAmountIn() public {
        ...
        // @audit configure in test the min/max treshold
        // @audit usdt is 1e6 how it behaves ?
        /*
            @audit-info
            tokenIn = usdt
            tokenOut= usd1
    ...
    function test_swap_WhenMintUSD1WithAmountOut() public {
        ...
        // Adds collateral to pass the reserve ratio checking
        _addCollateral(address(_usdt), ScalingUtils.scaleByDecimals(1e18, 18, _usdt.decimals()));
        deal(address(_usdt), address(this), amountIn);
    ...
    function test_swap_RedeemedAssetTokenWithUsingCollaterals() public {
        ...
        console.log("usd1Amount ", usd1Amount);
        console.log("usd1Amount ", (usd1Amount / 1e18));
        ...
        (, uint256 usdtAmount) = _unitas.swap(address(_usd1), address(_usdt), ISwapFunctions.AmountType.In, usd1Amount);
    ...
    function test_receivePortfolio_FailWhenNotPortfolio() public {
        vm.expectRevert(abi.encodeWithSignature("NotPortfolio(address)", address(this)));
    ...
    function testFuzz_swap_RandomFeeAndReserveRatioThreshold(
        uint24 feeNumerator,
        uint64 reserveRatioThreshold,
        uint128 scaledAmountIn,
        uint112 price
    ) public {
        // Zero or up to 99%
    ...
    function testFuzz_swap_WhenAssetTokenToUSD1WithAmountIn(uint128 scaledAmountIn, uint112 price, address account)
        public
    {
    ...
    function testFuzz_swap_WhenAssetTokenToUSD1WithAmountOut(uint128 amountOut, uint112 price, address account)
        public
    {
        _checkSkipAccount(account);
    ...
    function testFuzz_swap_WhenUSD1ToAssetTokenWithAmountIn(uint128 amountIn, uint112 price, address account) public {
        ...
        amountIn = uint128(
            bound(
                amountIn,
                (uint256(0.000001e18) * 1e18).ceilDiv(price)
                    + _calculateFeeByAmountWithoutFee(
                        (uint256(0.000001e18) * 1e18).ceilDiv(price), feeNumerator, _tokenManager.SWAP_FEE_BASE()
                    ),
                type(uint128).max
            )
        );
    ...
    function testFuzz_swap_WhenUSD1ToAssetTokenWithAmountOut(uint128 amountOut, uint112 price, address account)
        public
    {
        _checkSkipAccount(account);
    ...
    function testFuzz_swap_WhenUSD1ToStableTokenWithAmountOut(uint128 amountOut, uint112 price, address account)
        public
    {
        _checkSkipAccount(account);
        amountOut = uint128(bound(amountOut, 1, type(uint128).max));
    ...
    function testFuzz_swap_WhenStableTokenToUSD1WithAmountOut(uint128 amountOut, uint112 price, address account)
        public
    {
        _checkSkipAccount(account);
        amountOut = uint128(bound(amountOut, 1, type(uint128).max));
    ...
    function testFuzz_swap_WhenOneUserMultipleTimes(
        ...
        _addCollateral(
            address(_usdt), ScalingUtils.scaleByDecimals(uint256(times) * 2 * type(uint80).max, 18, _usdt.decimals())
        );
    ...
    function testFuzz_estimateSwapResult_WhenAssetTokenToUSD1WithAmountIn(uint128 scaledAmountIn, uint112 price)
        public
    {
        price = uint112(bound(price, 0.00000001e18, type(uint112).max));
        scaledAmountIn = uint128(
    ...
    function testFuzz_estimateSwapResult_AmountTypeReverseWhenAssetTokenToUSD1WithAmountInFirst(
        uint128 scaledAmountIn,
        uint112 price
    ) public {
        price = uint112(bound(price, 0.00000001e18, type(uint112).max));
        ...
        (uint256 amountIn2, uint256 amountOut2,, uint256 fee2,,) =
            _unitas.estimateSwapResult(tokenIn, tokenOut, ISwapFunctions.AmountType.Out, amountOut);
        ...
        assertLeDecimal(
            amountInDiff, amountTolerance, IERC20Metadata(tokenIn).decimals(), "amount in diff <= tolerance"
        );
    ...
    function testFuzz_estimateSwapResult_AmountTypeReverseWhenAssetTokenToUSD1WithAmountOutFirst(
        uint128 scaledAmountOut,
        uint112 price
    ) public {
        price = uint112(bound(price, 0.00000001e18, type(uint112).max));
        ...
        (uint256 amountIn2, uint256 amountOut2,, uint256 fee2,,) =
            _unitas.estimateSwapResult(tokenIn, tokenOut, ISwapFunctions.AmountType.In, amountIn);
        ...
        uint256 amountTolerance =
            amountOut == amountOut2 ? 0 : (10 ** (18 - IERC20Metadata(tokenIn).decimals()) * 1e18).ceilDiv(price);
        ...
        assertLeDecimal(
            amountOutDiff, amountTolerance, IERC20Metadata(tokenOut).decimals(), "amount out diff <= tolerance"
        );
        assertLeDecimal(feeDiff, feeTolerance, IERC20Metadata(tokenOut).decimals(), "fee diff <= tolerance");
    }

    function testFuzz_estimateSwapResult_AmountTypeReverseWhenUSD1ToAssetTokenWithAmountInFirst(
        uint128 scaledAmountIn,
        uint112 price
    ) public {
        ...
        // ceil min value of USDT / price
        uint256 tolerance =
            amountIn == amountIn2 ? 0 : (10 ** (18 - IERC20Metadata(tokenOut).decimals()) * 1e18).ceilDiv(price);
        uint256 amountTolerance =
            tolerance + _calculateFeeByAmountWithoutFee(tolerance, feeNumerator, _tokenManager.SWAP_FEE_BASE());
        uint256 amountInDiff = amountIn >= amountIn2 ? amountIn - amountIn2 : amountIn2 - amountIn;

        // When the results of amount out are different,
        // fee tolerance is the fee of the difference between them.
        uint256 feeTolerance =
            amountIn == amountIn2 ? 0 : _calculateFee(amountInDiff, feeNumerator, _tokenManager.SWAP_FEE_BASE());
        uint256 feeDiff = fee >= fee2 ? fee - fee2 : fee2 - fee;
    ...
    function testFuzz_estimateSwapResult_AmountTypeReverseWhenUSD1ToAssetTokenWithAmountOutFirst(
        uint128 scaledAmountOut,
        uint112 price
    ) public {
        price = uint112(bound(price, 0.00000001e18, type(uint112).max));
    ...
    function _assertSwap(SwapData memory data) internal {
        ...
        if (_tokenManager.getTokenType(data.tokenIn) == ITokenManager.TokenType.Asset) {
            ...
            uint256 reserveSubtracted =
                data.amountOut.min(_unitas.getReserve(data.tokenOut) - _unitas.getPortfolio(data.tokenOut));
            vm.expectEmit(true, true, true, true, address(_unitas));
    ...
    function _assertSwapWhenAssetTokenToUSD1(SwapData memory data) internal {
        ...
        if (reserveRatioThreshold >= 1e18) {
            ...
            collateral = _convertByFromPriceWhenRoundUp(data.tokenOut, data.tokenIn, collateral, price, 18);
    ...
    function _removeTokensAndPairs(address[] memory tokens, address[] memory pairTokensX, address[] memory pairTokensY)
        internal
    {
    ...
    function _isAccountExcluded(address account) internal view returns (bool) {
        return account == address(0) || account == address(_unitas) || account == address(_insurancePool)
            || account == address(_surplusPool) || account == address(_proxyAdmin);
    }
    ...
    function _calculateResultWhenAssetTokenToUSD1WithAmountIn(SwapData memory data)
        internal
        view
    ...
    function _calculateResultWhenAssetTokenToUSD1WithAmountOut(SwapData memory data)
        internal
        view
    ...
    function _calculateResultWhenUSD1ToAssetTokenWithAmountIn(SwapData memory data)
        internal
        view
        returns (uint256 amountOut, uint256 fee)
    {
    ...
    function _calculateResultWhenUSD1ToAssetTokenWithAmountOut(SwapData memory data)
        internal
        view
        returns (uint256 amountIn, uint256 fee)
    {
    ...
    function _calculateResultWhenUSD1ToStableTokenWithAmountIn(SwapData memory data)
        internal
        view
        returns (uint256 amountOut, uint256 fee)
    {
    ...
    function _calculateResultWhenUSD1ToStableTokenWithAmountOut(SwapData memory data)
        internal
        view
        returns (uint256 amountIn, uint256 fee)
    {
    ...
    function _calculateResultWhenStableTokenToUSD1WithAmountIn(SwapData memory data)
        internal
        view
        returns (uint256 amountOut, uint256 fee)
    {
    ...
    function _calculateResultWhenStableTokenToUSD1WithAmountOut(SwapData memory data)
        internal
        view
        returns (uint256 amountIn, uint256 fee)
    {
...
contract UnitasHarness is Unitas {
    ...
    function exposed_getSwapResult(
