...
contract PoolBalancesTest is Test {
    ...
    function testFuzz_receivePortfolio_Received(
    ...
    function testFuzz_sendPortfolio_SentWhenAmountGreaterThanPoolBalance(
        uint256 balanceAmount,
        uint256 inputAmount,
        address account
    ) public {
    ...
    function _assertSentPortfolio(address token, address receiver, uint256 inputAmount, uint256 actualAmount)
        internal
    {
    ...
    function _assertPortfolioEq(address token, uint256 portfolio) internal {
        ...
        assertEq(
            _poolBalances.exposed_portfolio(token), _poolBalances.exposed_getPortfolio(token), "portfolio eq getter"
        );
    }
}
