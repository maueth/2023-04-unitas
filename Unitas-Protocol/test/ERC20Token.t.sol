...
contract ERC20TokenTest is Test {
    ...
    function testMint_ShouldFail_WhenCallerIsNonMinter() public {
        vm.startPrank(userA);
        vm.expectRevert(abi.encodeWithSignature("NotMinter(address)", userA));
    ...
    function testMint_ShouldFail_WhenRecipientIsBlacklisted() public {
        ...
        vm.startPrank(Minter);
        vm.expectRevert(abi.encodeWithSignature("Blacklisted(address)", userA));
        token.mint(userA, 2e18);
        vm.stopPrank();
    }
    ...
    function testPause_ShouldFail_WhenCallerIsNonGuardian() public {
        vm.startPrank(userA);
        vm.expectRevert(abi.encodeWithSignature("NotGuardian(address)", userA));
        token.pause();
        vm.stopPrank();
    }
    ...
    function testTransfer_ShouldFail_WhenRecipientIsBlacklisted() public {
        ...
        vm.startPrank(userA);
        vm.expectRevert(abi.encodeWithSignature("Blacklisted(address)", userB));
        token.transfer(userB, 2e18);
        vm.stopPrank();
    }
    ...
    function testTransferFrom_ShouldFail_WhenCallerIsBlacklisted() public {
        ...
        vm.startPrank(userC);
        vm.expectRevert(abi.encodeWithSignature("Blacklisted(address)", userC));
        token.transferFrom(userA, userB, 0.7e18);
        vm.stopPrank();
    }
    ...
    function testSetMinter_ShouldFail_WhenCallerIsNonGovernor() public {
        vm.startPrank(userA);
        vm.expectRevert(abi.encodeWithSignature("NotGovernor(address)", userA));
        token.setMinter(userA, Minter);
        vm.stopPrank();
    }
